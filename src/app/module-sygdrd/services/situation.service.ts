...
export class SituationService {
   ...
   * @returns 
   */
   getFt(page:Page,dateDebut:string, dateFin:string): Observable<any> {
    let url = `${environment.defaultauth}/api/bank-statements/ftInDate?dateDebut=${dateDebut}&dateFin=${dateFin}&page=${page.pageNumber}&size=${page.size}`;
    console.log(url)
 ...
 */
  ...
  /**
   * Methode qui permet d'uploader un fichier Prevision
   * @param file 
   * @returns 
   */
  ...
  getSituationByCodeBudget(dateDebut:string, dateFin:string): Observable<SituationCodeBudget[]> {
    let url = `${environment.defaultauth}/codeBudget?dateDebut=${dateDebut}&dateFin=${dateFin}`;
    console.log(url)
    return this.http.get<SituationCodeBudget[]>(url, {
      headers: new HttpHeaders({
        "Content-Type": "application/json",
      }),
    });
