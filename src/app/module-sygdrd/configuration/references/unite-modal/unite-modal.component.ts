...
import {UniteTabsComponent} from "../unite-tabs/unite-tabs.component";
...
export class UniteModalComponent implements OnInit {
  ...
  uniteTabsComponent!: UniteTabsComponent;
  unite!: Unite;
  ...
  typesUnites: string[] = [];
  unites: Unite[] = [];
  isRootUnit: boolean = false;  // Nouvelle propriété pour identifier une unité racine

  constructor(
      public uniteService: UniteService,
      public enumService: EnumService,
      private fb: FormBuilder,
      @Host() uniteTabsComponent: UniteTabsComponent
  ) {
    this.uniteTabsComponent = uniteTabsComponent;
  }
  ...
  ngOnInit(): void {
    ...
    if (this.innerUnite != null) {
      ...
      // Vérifier si c'est une unité racine (sans parent)
  ...
  Onsave(): void {
    ...
    // Si c'est une unité racine, conserver la valeur originale du parent (qui est nulle)
    if (this.isRootUnit) {
  ...
  private showSuccessMessage(message: string): void {
    Swal.fire({
      ...
      this.uniteTabsComponent?.uniteOrganigrammeComponent?.closeModal();
      this.uniteTabsComponent?.uniteOrganigrammeComponent?.loadData();
    });
  }
