...
@Component({
  ...
  templateUrl: './bon-bolorer.component.html',
  styleUrl: './bon-bolorer.component.scss'
})
export class BonBolorerComponent {
 bonSortieForm: FormGroup;
   @Input() childProperty = new Declaration_1();
   SysrevBonBolorer = new BonBolorer();
   profil: any;
   fileToUpload: any;
   FileProfileId: any;
  ...
  constructor(private fb: FormBuilder,private fileUploadService:UploadService,private DeclarationService:DeclarationService,@Host() private indexCmpBon:IndexBonComponent) {
  this.bonSortieForm = this.fb.group({
   reference: [''],                        // ← ENLEVÉ Validators.required
   dateValidite: ['', Validators.required],
   dateEmission: ['', Validators.required],
   fileUuid: [''],                         // ← DÉJÀ sans Validators.required
    uuidDeclaration:[''],
  });
  this.indexCmpBonEditor = indexCmpBon
   ...
   ngOnInit(): void {
     ...
     if(this.childProperty.bonSortieBolorerDto != null){
       this.display(this.childProperty.bonSortieBolorerDto)
     }else{
  ...
  if (this.bonSortieForm.valid) {
    this.SysrevBonBolorer.dateEmission = this.bonSortieForm.value.dateEmission
    this.SysrevBonBolorer.dateValidite = this.bonSortieForm.value.dateValidite;
    
    // ✅ CHAMPS FACULTATIFS - Gérer les valeurs vides
    this.SysrevBonBolorer.refbonsortie = this.bonSortieForm.value.reference || '';
    this.SysrevBonBolorer.declarationUuid = this.childProperty.uuid;
    this.SysrevBonBolorer.fileUuid = this.FileProfileId || '';
    
    console.log(this.SysrevBonBolorer)
    this.DeclarationService.createBolorer(this.SysrevBonBolorer).subscribe(res => {
      let timerInterval: any;
      Swal.fire({
        title: `Bon de manutationnaire enregistré avec succes`,
        icon: 'success',
        timer: 2000,
   ...
   display(SysrevBonBolorer:BonBolorer){
    this.bonSortieForm.patchValue({
      reference: SysrevBonBolorer.refbonsortie,
      dateValidite: SysrevBonBolorer.dateValidite,
      dateEmission: SysrevBonBolorer.dateEmission,
      fileUuid: [''],
       uuidDeclaration:[''],
     })
