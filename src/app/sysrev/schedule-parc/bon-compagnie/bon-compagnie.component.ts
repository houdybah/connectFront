...
@Component({
  selector: 'app-bon-compagnie',
  templateUrl: './bon-compagnie.component.html',
  styleUrl: './bon-compagnie.component.scss'
})
export class BonCompagnieComponent implements OnInit{
  bonSortieForm: FormGroup;
  @Input() childProperty = new Declaration_1();
  SysrevBonCompagnie = new BonCompagnie();
  profil: any;
  fileToUpload: any;
  FileProfileId: any;
  ...
  pdfIcon:string = `<svg 
  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="50" height="50" viewBox="0 0 256 256" xml:space="preserve">
  ...
  indexCmpBonEditor!:IndexBonComponent;

 constructor(private fb: FormBuilder,private fileUploadService:UploadService,
  private DeclarationService:DeclarationService,@Host() private indexCmpBon:IndexBonComponent) {
  this.bonSortieForm = this.fb.group({
    refCompagnie: [''],                      // ← ENLEVÉ Validators.required
    ...
    fileUuid:[''],                          // ← ENLEVÉ Validators.required
  });

  this.indexCmpBonEditor = indexCmpBon
}
  ngOnInit(): void {
   
  
 //console.log(this.compagnies)
    if(this.childProperty.bonCompagnieDto != null){
      this.display(this.childProperty.bonCompagnieDto)
  ...
  if (this.bonSortieForm.valid) {
    this.SysrevBonCompagnie.compagnie = this.bonSortieForm.value.compagnie
    this.SysrevBonCompagnie.dateEmission = this.bonSortieForm.value.dateEmission;
    this.SysrevBonCompagnie.dateValidite = this.bonSortieForm.value.dateValidite
    this.SysrevBonCompagnie.declarationUuid = this.childProperty.uuid
    
    // ✅ CHAMPS FACULTATIFS - Gérer les valeurs vides
    this.SysrevBonCompagnie.refCompagnie = this.bonSortieForm.value.refCompagnie || '';
    this.SysrevBonCompagnie.typeLivraison = this.bonSortieForm.value.typeLivraison
    this.SysrevBonCompagnie.fileUuid = this.FileProfileId || '';
    
    this.DeclarationService.createCompagnie(this.SysrevBonCompagnie).subscribe(res => {
      let timerInterval: any;
      Swal.fire({
        title: `Bon de la compagnie enregistré avec succes`,
        icon: 'success',
        timer: 2000,
        ...
        this.indexCmpBonEditor.getDeclarations();
      })
    })
    
    console.log('Bon de sortie soumis :', this.bonSortieForm.value);
  } else {
    console.log('Formulaire invalide');
  }
}

  display(SysrevBonCompagnie:BonCompagnie){
    console.log(SysrevBonCompagnie)
    
      this.bonSortieForm.patchValue({
        refCompagnie:SysrevBonCompagnie.refCompagnie,
        dateEmission: SysrevBonCompagnie.dateEmission,
        dateValidite: SysrevBonCompagnie.dateValidite,
        compagnie: SysrevBonCompagnie.compagnie,
        typeLivraison: SysrevBonCompagnie.typeLivraison,
        fileUuid:SysrevBonCompagnie.fileUuid,
      })
      this.bonSortieForm.disable()
      this.isDisplay = true;
  ...
  checkDate(){
    if(this.bonSortieForm.value.dateEmission <= this.bonSortieForm.value.dateValidite){
       
    }else{
        Swal.fire({
          icon: 'error',
          title: 'Erreur',
          text: 'La date de validite doit etre superieur a la date d\'emission',
        })
    }
  }
}
