...
@Component({
  selector: 'app-situation-delaration-par-code-dec',
  templateUrl: './situation-delaration-par-code-dec.component.html',
  styleUrls: ['./situation-delaration-par-code-dec.component.scss']
})
export class SituationDelarationParCodeDecComponent implements OnInit {
  ...
  initForm(): void {
    this.nifForm = this.fb.group({
      ...
      codeDec: ['', Validators.required],
      dateDebut: ['', Validators.required],
      dateFin: ['', Validators.required],
      statut: ['ALL', []], // Valeur par défaut "ALL"
...
dateRangeValidator(group: FormGroup) {
  ...
  getAllNifDeclarationParCodeDec(nif: string, codeDec: string, statut: string, dateDebut: string, dateFin: string): void {
    this.errorMessage = '';
    this.isLoading = true;
    
    console.log("🔹 Données envoyées à l'API :", { nif, codeDec, statut, dateDebut, dateFin, page: this.page });
    ...
    // Assurer que statut a toujours une valeur valide
    const statutValue = statut || 'ALL';

    this.situationDeclarationService.getDeclarationsByNifsParCodeDec(
      this.page, 
      '', 
      nif, 
      codeDec, 
      ...
      next: (pagedData: PagedData<SituationDeclaration>) => {
        ...
        // Générer le tableau des numéros de page pour l'affichage de la pagination
  ...
  onSubmit(): void {
    ...
    const { nif, codeDec, statut, dateDebut, dateFin } = this.nifForm.value;
    
    // Réinitialiser la pagination lors d'une nouvelle recherche
    this.initPage();
  
    this.getAllNifDeclarationParCodeDec(nif, codeDec, statut, dateDebut, dateFin);
  }

  // Méthode pour générer les numéros de page pour la pagination
  ...
  /**
 * Handles changes to the page size selection dropdown
 * Updates pagination and reloads data with new page size
 */
onPageSizeChange(): void {
  ...
  // If form is valid, reload the data with new page size
  if (this.nifForm.valid) {
    const { nif, codeDec, statut, dateDebut, dateFin } = this.nifForm.value;
    this.getAllNifDeclarationParCodeDec(nif, codeDec, statut, dateDebut, dateFin);
  }
  
  // Regenerate pagination based on new page size
  this.generatePagination();
}
  ...
  exportToPdf(): void {
    ...
    this.situationDeclarationService.exportToDPdf(
      formattedDateDebut,
      formattedDateFin,
      nif || '',
      statut,
      ...
      next: (blob: Blob) => {
        this.downloadFile(blob, `declarations_code_${codeDec}_${formattedDateDebut}_${formattedDateFin}.pdf`);
        this.isLoading = false;
      },
      error: (error: any) => {
  ...
  exportToExcel(): void {
    ...
    this.situationDeclarationService.exportToDExcel(
      formattedDateDebut,
      formattedDateFin,
      nif || '',
      statut,
      ...
      next: (blob: Blob) => {
        this.downloadFile(blob, `declarations_code_${codeDec}_${formattedDateDebut}_${formattedDateFin}.xls`);
        this.isLoading = false;
      },
      error: (error: any) => {
