...
@Component({
  selector: 'app-situation-delaration-par-bureau',
  templateUrl: './situation-delaration-par-bureau.component.html',
  styleUrls: ['./situation-delaration-par-bureau.component.scss']
})
export class SituationDelarationParBureauComponent implements OnInit {
...
dateRangeValidator(group: FormGroup) {
  ...
  getAllNifDeclarationParBureau(nif: string, bureau: string, statut: string, dateDebut: string, dateFin: string): void {
    this.errorMessage = '';
    this.isLoading = true;
    
    console.log("🔹 Données envoyées à l'API :", { nif, bureau, statut, dateDebut, dateFin, page: this.page });
    ...
    this.situationDeclarationService.getDeclarationsByNifsParBureau(
      this.page, 
      '', // key vide 
      nif, 
      bureau,
 ...
 onSubmit(): void {
  ...
  if (this.nifForm.invalid) {
    ...
    Object.keys(this.nifForm.controls).forEach(key => {
      this.nifForm.get(key)?.markAsTouched();
    });

    // Message d’erreur et désactivation du spinner
    this.errorMessage = "Veuillez remplir tous les champs obligatoires.";
    this.isLoading = false;
    return;
  }

  // Active le spinner pendant le traitement
  this.isLoading = true;

  // Récupère les valeurs du formulaire
  const { nif, bureau, statut, dateDebut, dateFin } = this.nifForm.value;

  // Réinitialise la pagination
  this.initPage();

  // Appelle la méthode de récupération des données
  this.getAllNifDeclarationParBureau(nif, bureau, statut || 'ALL', dateDebut, dateFin);
}
  ...
  goToPage(pageNumber: number): void {
    ...
    const { nif, bureau, statut, dateDebut, dateFin } = this.nifForm.value;
    this.getAllNifDeclarationParBureau(
      nif || '', 
      bureau || '', 
      statut || 'ALL', 
      dateDebut || '', 
  ...
  exportToPdf(): void {
    ...
    this.situationDeclarationService.exportToPdf(
      formattedDateDebut,
      formattedDateFin,
      nif || '',
      statut || 'ALL',
      ...
      next: (blob: Blob) => {
        this.downloadFile(blob, `declarations_bureau_${bureau}_${formattedDateDebut}_${formattedDateFin}.pdf`);
        this.isLoading = false;
      },
      error: (error: any) => {
  ...
  exportToExcel(): void {
    ...
    this.situationDeclarationService.exportToExcel(
      formattedDateDebut,
      formattedDateFin,
      nif || '',
      statut || 'ALL',
      ...
      next: (blob: Blob) => {
        this.downloadFile(blob, `declarations_bureau_${bureau}_${formattedDateDebut}_${formattedDateFin}.xls`);
        this.isLoading = false;
      },
      error: (error: any) => {
  ...
  private handleExportError(error: any, context: string): void {
    ...
    if (error.error instanceof Blob) {
      // Si l'erreur est un Blob, essayez de le lire comme texte
      ...
      reader.onload = () => {
        try {
          ...
          try {
            ...
            this.errorMessage = errorText || `Une erreur est survenue lors de l'exportation en ${context}.`;
          }
        } catch (e) {
